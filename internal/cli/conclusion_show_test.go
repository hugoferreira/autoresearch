...
var _ = testkit.Spec("TestConclusionShowJSON_JoinsObservationArtifacts", func(t testkit.T) {
	saveGlobals(t)
	dir := t.TempDir()
...
var _ = testkit.Spec("TestConclusionShowJSON_OmitsObservationArtifactsWhenNoObservations", func(t testkit.T) {
	saveGlobals(t)
	dir := t.TempDir()
	s, err := store.Create(dir, store.Config{
		Build: store.CommandSpec{Command: "true"},

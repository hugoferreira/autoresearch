...
var _ = testkit.Spec("TestInitializeLessonState", func(t testkit.T) {
...
var _ = testkit.Spec("TestSyncHypothesisLessons", func(t testkit.T) {
	s := mustCreateCLIStore(t)
	now := time.Now().UTC()
...
var _ = testkit.Spec("TestSyncHypothesisLessons_ReclassifiesMalformedSystemLessons", func(t testkit.T) {
	s := mustCreateCLIStore(t)
	now := time.Now().UTC()
...
var _ = testkit.Spec("TestWriteWorktreeBrief_ExcludesNonSteeringLessons", func(t testkit.T) {
	s := mustCreateCLIStore(t)
	now := time.Now().UTC()

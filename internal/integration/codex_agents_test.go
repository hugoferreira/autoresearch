...
var _ = testkit.Spec("TestEmbeddedCodexAgents_RewrittenForCodex", func(t testkit.T) {
...
var _ = testkit.Spec("TestEmbeddedCodexAgents_NotebookPropagation", func(t testkit.T) {
	agents, err := integration.EmbeddedCodexAgents()
	if err != nil {
		t.Fatal(err)
	}
...
var _ = testkit.Spec("TestCodexDelegationHandoffContract", func(t testkit.T) {
	agents, err := integration.EmbeddedCodexAgents()
	if err != nil {
		t.Fatal(err)
	}
...
var _ = testkit.Spec("TestInstallCodexAgents", func(t testkit.T) {
	dir := t.TempDir()
...
var _ = testkit.Spec("TestInstallCodexAgents_RemovesLegacyMarkdown", func(t testkit.T) {
	dir := t.TempDir()
...
var _ = testkit.Spec("TestInstallCodexAgents_PreservesSiblingFiles", func(t testkit.T) {
	dir := t.TempDir()
	agentsDir := filepath.Join(dir, ".codex", "agents")
	if err := os.MkdirAll(agentsDir, 0o755); err != nil {
		t.Fatal(err)

...
- Treat the target project's main checkout as read-only during research. Experiment and harness changes belong in experiment worktrees. Setup refreshes under ` + "`AGENTS.md`" + `, ` + "`.claude/`" + `, ` + "`.codex/`" + `, ` + "`.gitignore`" + `, or ` + "`.research/`" + ` are separate maintenance, not part of a hypothesis cycle. If ` + "`autoresearch cycle-context --json`" + `, ` + "`status`" + `, or ` + "`dashboard`" + ` reports ` + "`main_checkout_dirty_paths`" + `, stop and surface that maintenance explicitly instead of patching bootstrap/harness files in place.
...
- Never read agent runtime harness caches for research state. Claude Code caches under ` + "`~/.claude/projects/`" + ` (especially ` + "`tool-results/`" + `) and Codex session/log caches under ` + "`~/.codex/`" + ` can be stale, huge transcripts. Use ` + "`autoresearch`" + ` read verbs and artifact readers instead.
- Use ` + "`autoresearch observation show O-XXXX --include-raw --json`" + ` when you need sample-level stdout or exit codes from an observation; do not inspect ` + "`.research/observations/*.json`" + ` directly.
- Capture reasoning, not just measurements: pass ` + "`--rationale`" + ` on ` + "`hypothesis add`" + `, ` + "`--design-notes`" + ` on ` + "`experiment design`" + `, ` + "`--impl-notes`" + ` on ` + "`experiment implement`" + `, and ` + "`--interpretation`" + ` on ` + "`conclude`" + `. Record cumulative lessons with ` + "`autoresearch lesson add`" + ` on decisive conclusions, and read ` + "`autoresearch cycle-context --json`" + `'s ` + "`active_lessons`" + ` before proposing new hypotheses — the loop should not re-derive what it already knows.
- Run ` + "`autoresearch experiment preflight <E-id> --json`" + ` after ` + "`experiment design`" + ` and before ` + "`experiment implement`" + `. Treat preflight errors as redesign blockers; account for warnings in the design notes or handoff.
- Use ` + "`autoresearch review-packet <C-id> --json`" + ` plus ` + "`autoresearch conclusion lint <C-id> --json`" + ` as the gate reviewer's starting point. Packet and lint output are read-only evidence bundles, not steering surfaces.
- Choose lesson scope conservatively: if a lesson comes from a specific H-/E-/C- chain, depends on that chain's conditions, or recommends continuing an optimization direction, keep it hypothesis-scoped. Reserve ` + "`scope=system`" + ` for cross-goal invariants, harness behavior, environment quirks, or measurement caveats. If unsure, prefer hypothesis scope.
...
- ` + "`lesson list`" + ` defaults to active lessons; use ` + "`--status all`" + ` only for audit/history. Lessons sourced from a decisive conclusion are provisional until gate review. Do not use ` + "`--inspired-by`" + ` to cite lessons whose source chain is not currently ` + "`reviewed_decisive`" + `; ` + "`--allow-invalidated`" + ` is only for explicit retrospective contrast.
...
- Humans steer through the main Codex conversation. Read-only views like ` + "`cycle-context`" + `, ` + "`dashboard`" + `, ` + "`log`" + `, ` + "`tree`" + `, ` + "`frontier`" + `, ` + "`lesson list`" + `, and ` + "`report`" + ` are windows onto state, not steering surfaces.
